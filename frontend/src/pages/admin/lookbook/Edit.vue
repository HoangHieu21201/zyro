...
const fetchData = async () => {
  ...
  try {
    const [resLB, resProd, resCats, resBrands] = await Promise.all([
      api.get(`/admin/lookbooks/${lookbookId}`),
    ...
    // Đã ánh xạ dữ liệu usage_limit
    form.value.usage_limit = lb.usage_limit === null || lb.usage_limit === undefined ? '' : lb.usage_limit;
...
const submitLookbook = async () => {
  ...
  // ĐẨY GIỚI HẠN LÊN BACKEND KHI CẬP NHẬT
  if (form.value.usage_limit !== '' && form.value.usage_limit !== null && form.value.usage_limit !== undefined) {
      ...
      // Để trống chuỗi báo hiệu cho backend xóa Limit (thành Vô Hạn)
      formData.append('usage_limit', '');
  }
  ...
  try {
    const res = await api.post(`/admin/lookbooks/${lookbookId}`, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
