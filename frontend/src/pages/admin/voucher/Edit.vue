<!-- File: frontend/src/pages/admin/voucher/Edit.vue -->
<template>
      ...
      <div class="row mb-4 align-items-center">
        <div class="col-md-8 d-flex align-items-center mb-3 mb-md-0">
          ...
          <div class="d-flex flex-column">
            <h3 class="fw-bold text-dark dark:text-white mb-0">Cập Nhật Mã Khuyến Mãi</h3>
            <p class="text-muted dark:text-gray-400 small mb-0 mt-1">Chỉnh sửa chi tiết chiến dịch giảm giá</p>
          </div>
        </div>
          ...
          <div class="col-xl-8 col-lg-7">
            ...
            <div class="card border-0 shadow-sm rounded-4 dark:bg-[#1a2533] p-4 mb-4">
              ...
              <div class="row g-4 mb-3">
                <div class="col-md-7">
                  ...
                  <input type="text" class="form-control bg-light dark:bg-[#212529] dark:text-white border-0 shadow-sm-hover" v-model="form.name" required>
                ...
                <div class="col-md-5">
                  ...
                  <div class="input-group shadow-sm-hover">
                    <input type="text" class="form-control font-monospace text-uppercase bg-light dark:bg-[#212529] dark:text-white border-0" v-model="form.code" @input="formatCode" required>
                ...
                <div class="col-md-12" v-if="form.discount_type === 'percent'">
                   <div class="p-3 bg-light dark:bg-[#212529] border border-light-subtle dark:border-gray-700 rounded-3">
                     ...
                     <div class="input-group shadow-sm-hover w-50">
                       <input type="number" class="form-control border-0 bg-white dark:bg-[#1a2533]" v-model.number="form.max_discount_amount" min="0" placeholder="Không giới hạn">
            ...
            <div class="card border-0 shadow-sm rounded-4 dark:bg-[#1a2533] p-4">
              ...
              <div class="row g-4">
                ...
                <!-- ĐÃ NÂNG CẤP: Bảng Multi-select thay vì bắt Admin tự nhập ID -->
          ...
          <div class="col-xl-4 col-lg-5">
            ...
            <div class="card border-0 shadow-sm rounded-4 dark:bg-[#1a2533] p-4 mb-4">
              ...
              <div class="mb-3">
                ...
                <small class="text-success fw-bold d-block mt-1">Đã dùng: {{ usageCount }} lượt</small>
              </div>
            ...
            <div class="card border-0 shadow-sm rounded-4 dark:bg-[#1a2533] p-4">
              ...
              <div>
                ...
                <select class="form-select bg-light dark:bg-[#212529] dark:text-white border-0 shadow-sm-hover" v-model="form.status">
                  ...
                  <option value="expired">Hết hạn (Kết thúc sớm)</option>
                </select>
              </div>
            </div>
...
const fetchData = async () => {
  try {
    ...
    form.value.discount_type = v.discount_type;
    form.value.discount_value = v.discount_value;
    form.value.max_discount_amount = v.max_discount_amount;
    form.value.min_spend = v.min_spend;
    form.value.apply_type = v.apply_type;
    form.value.usage_limit = v.usage_limit;
    form.value.usage_limit_per_user = v.usage_limit_per_user;
    form.value.is_public = v.is_public === 1 || v.is_public === true;
    form.value.status = v.status;
    
    // Cắt chuỗi để nạp vào input datetime-local
    form.value.start_time = formatDateForInput(v.start_time);
    form.value.end_time = formatDateForInput(v.end_time);
    
    usageCount.value = v.usage_count || 0;

    if (v.conditions && Array.isArray(v.conditions)) {
        selectedConditions.value = v.conditions;
    }
    ...
    Swal.fire('Lỗi', 'Không tìm thấy thông tin Voucher', 'error');
...
const submitVoucher = async () => {
  ...
  try {
    ...
    } else { Swal.fire('Lỗi', e.response?.data?.message || 'Lỗi cập nhật Voucher.', 'error'); }
