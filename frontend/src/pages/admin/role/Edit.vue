...
<template>
  <div class="role-edit-wrapper pb-5 mb-5">
    <div class="container-fluid py-4">
      ...
      <div v-if="isLoadingRole" class="text-center py-5 text-muted">
        <span class="spinner-border text-urban mb-2"></span>
        <p>Đang tải thông tin Role...</p>
      </div>

      <div v-else class="card border-0 shadow-sm rounded-4 dark:bg-[#1a2533] p-4">
          ...
          <div class="mb-4 p-4 bg-light dark:bg-[#212529] rounded-4 border border-light-subtle dark:border-gray-700 shadow-sm">
            ...
            <div class="d-flex align-items-stretch gap-4 flex-wrap flex-md-nowrap">
                <div class="input-group shadow-sm flex-shrink-0 border border-urban rounded" style="width: 140px; height: fit-content;">
                    <button class="btn btn-light bg-white dark:bg-[#1a2533] border-end dark:border-gray-700" type="button" @click="roleForm.level > 1 && roleForm.id !== 1 ? roleForm.level-- : null" :disabled="roleForm.id === 1">
                    ...
                    <button class="btn btn-light bg-white dark:bg-[#1a2533] border-start dark:border-gray-700" type="button" @click="roleForm.level < 10 && roleForm.id !== 1 ? roleForm.level++ : null" :disabled="roleForm.id === 1">
            ...
            <div class="mt-3">
                ...
                <small class="text-muted dark:text-gray-400 d-block fst-italic" v-else><i class="bi bi-info-circle-fill me-1"></i>Số Level càng nhỏ thì quyền hạn càng lớn.</small>
            </div>
          </div>
...
const errors = ref({}); 
const roleForm = ref({ id: null, label: '', value: '', badge_class: '', level: 5 });
...
const fetchRole = async () => {
  try {
    ...
    Swal.fire('Lỗi', 'Không tìm thấy thông tin Role!', 'error');
...
const saveRole = async () => {
  ...
  try {
    ...
    if (err.response && err.response.data && err.response.data.errors) {
      ...
      Swal.fire('Lỗi', err.response?.data?.message || 'Không thể cập nhật Role', 'error');
    }
  } finally { 
