      ...
      <div class="row mb-4 align-items-center">
        <div class="col-md-8 d-flex align-items-center">
          ...
          <div class="d-flex flex-column">
            ...
            <p class="text-muted dark:text-gray-400 small mb-0 mt-1">Chỉnh sửa thông tin hiện tại, dọn dẹp các thuộc tính rác</p>
          </div>
        </div>
      </div>
            ...
            <div v-show="currentStep === 2" class="animation-fade-in">
              <div class="card border border-secondary-subtle dark:border-gray-700 shadow-sm rounded-4 overflow-hidden mb-4">
                <div class="card-header bg-white dark:bg-[#212529] border-bottom dark:border-gray-700 p-3 d-flex justify-content-between align-items-center flex-wrap gap-3">
                  ...
                  <!-- ĐÃ NÂNG CẤP: KHU VỰC THÊM CỘT THUỘC TÍNH (QUICK ADD) -->
                  <div class="d-flex align-items-center flex-wrap gap-2">
                    ...
                    <!-- Select cho các thuộc tính còn lại (Nếu nhiều quá) -->
                      ...
                      <div class="d-flex flex-wrap gap-3">
                        ...
                        <div class="position-relative border rounded-4 overflow-hidden shadow-sm dark:border-gray-600 hover-scale"
                          ...
                          <button type="button" class="btn btn-danger btn-sm position-absolute top-0 end-0 m-1 rounded-circle p-0 d-flex align-items-center justify-content-center shadow-lg transition-all"
                            style="width: 26px; height: 26px;" @click.stop="removeGalleryImage(index, img.id)">
              ...
              <div class="mb-4">
                ...
                <div class="row g-3 p-3 bg-light dark:bg-[#212529] rounded-4 border dark:border-gray-700 shadow-inner">
                  ...
                  <div class="col-md-6 border-start dark:border-gray-600 ps-md-4">
                    <h6 class="small text-muted fw-bold text-center border-bottom dark:border-gray-600 pb-2 mb-3"><i class="bi bi-sort-numeric-down me-1"></i>Chữ Số (Tăng dần)</h6>
                    <div class="d-flex flex-column gap-2 pe-1 custom-scrollbar-y" style="max-height: 250px; overflow-y: auto;">
...
// --- LOGIC TÌM KIẾM TRONG DROPDOWN (MỚI) ---
...
const toggleDropdown = (rowIndex, attrId) => {
  ...
  if (activeDropdown.value === id) {
      ...
      attrSearchQuery.value = ''; // Tự động reset search khi mở popup mới
  }
};
...
const canProceedToStep2 = computed(() => {
  return form.value.name && form.value.category_id && form.value.base_price !== '' && form.value.base_price >= 1000;
});
...
const getSortedValues = (attrId) => {
  ...
  const numeric = [];
  filtered.forEach(v => {
      // Phân tách khôn khéo: Nếu là số (bao gồm số thực) thì ném vào mảng numeric
  ...
  // Tự động xếp A-Z chuẩn Tiếng Việt
  alpha.sort((a, b) => a.value.localeCompare(b.value, 'vi'));
  // Tự động xếp từ bé đến lớn
...
const popularAttributes = computed(() => {
    // Ưu tiên hiển thị ngay các thuộc tính thiết yếu của thời trang
    ...
    ).slice(0, 4); // Show tối đa 4 nút đẹp gọn
});
...
const submitProduct = async () => {
  ...
  try {
    ...
    const res = await api.post(`/admin/products/${productId}`, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
...
const fetchData = async () => {
  try {
    ...
    if (p.images && p.images.length > 0) {
      galleryPreviews.value = p.images.map(img => ({ id: img.id, url: getImageUrl(img.image_url), file: null }));
    }
