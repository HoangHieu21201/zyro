<template>
  <Teleport to="body">
    ...
    <!-- ĐÃ FIX: Bọc toàn bộ bằng 1 thẻ div root duy nhất để Vue không bị lú Fragment -->
    <div class="compare-modal-root-wrapper">
      ...
      <!-- POPUP BẢNG SO SÁNH FULL MÀN HÌNH -->
...
// ĐÃ FIX: Khai báo để hứng trọn những Props và Emits "thừa" mà cha ném xuống
// Việc này giúp dập tắt hoàn toàn 4 dòng Warning đỏ chót trong Console!
...
const removeAndCheck = (index) => {
  ...
  emit('remove', index); // Bắn tín hiệu lên cha (Dù không cần thiết do đã có Pinia, nhưng bắn để chuẩn rule)
...
const clearAll = () => {
  ZyroSwal.confirmDelete('tất cả sản phẩm so sánh').then((result) => {
    if (result.isConfirmed) {
      compareStore.clear();
      emit('clear'); // Bắn tín hiệu lên cha
      showModal.value = false;
